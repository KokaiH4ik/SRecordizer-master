...
namespace SRecordizer
{
    public partial class SRecordView : DockContent
    {
        ...
        {
            toollinenumberboxDEC.Text= "";
            toollinenumberboxDEC.Visible = true;
            toollinenumberboxDEC.Focus();
        }

        private void toollinenumberboxDEC_KeyPress(object sender, KeyPressEventArgs e)
        {
                ...
                if (e.KeyChar == '\r')
                {
                    try
                    {
                        int num = System.Convert.ToInt32(toollinenumberboxDEC.Text);
                        GoToLine(num);
                        editContextMenu.Visible = false;
                    }
                    catch
                    {
        ...
        private void jumpToAdresHEXToolStripMenuItem_Click(object sender, EventArgs e)
        {
            toollinenumberboxHEX.Text = "";
            toollinenumberboxHEX.Visible = true;
            toollinenumberboxHEX.Focus();
        }

        private void toollinenumberboxDEC_Leave(object sender, EventArgs e)
        {
            editContextMenu.Visible = false;
        }

        private void toollinenumberboxHEX_KeyPress(object sender, KeyPressEventArgs e)
        {
            
                if (e.KeyChar == '\r')
                {
                    int addr = System.Convert.ToInt32(toollinenumberboxHEX.Text, 16);
                    GoToAddress(addr);
                    editContextMenu.Visible = false;
                }
        }
        ...
        private void countToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if(s19ListView.SelectedObjects.Count!=0)
            {
                StringBuilder sb = new StringBuilder();
                foreach (int selectedIndex in s19ListView.SelectedIndices)
                {
                    ListViewItem item = s19ListView.Items[selectedIndex];
                    sb.Append(item.SubItems[4].Text);

                    sb.Append(Environment.NewLine);
                }

                string copiedData = sb.ToString();
                Thread viewerThread = new Thread(delegate ()
                {
                    Form1 f1 = new Form1(copiedData);
                    f1.ShowDialog();
                });
